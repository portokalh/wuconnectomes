...
def pca_patchloop_test(patch_radius, arr, arr_shape, mask, jx1, tau_factor, dim, is_svd, var, calc_sigma=True, verbose=False):
...
def genpca_parallel(arr, sigma=None, mask=None, patch_radius=2, pca_method='eig',
    ...
    for x in len(Xest):
        ...
        #ix1 = i[x] - patch_radius
        #ix2 = i[x] + patch_radius + 1
        #jx1 = j[x] - patch_radius
        #jx2 = j[x] + patch_radius + 1
        #kx1 = k[x] - patch_radius
        #kx2 = k[x] + patch_radius + 1
        ...
        theta[ix1:ix2, jx1:jx2, kx1:kx2] += this_theta[x]
        thetax[ix1:ix2, jx1:jx2, kx1:kx2] += Xest[x] * this_theta[x]
        if calc_sigma:
            var[ix1:ix2, jx1:jx2, kx1:kx2] += this_var[x] * this_theta[x]
            thetavar[ix1:ix2, jx1:jx2, kx1:kx2] += this_theta[x]
        else:
            var = 0
            thetavar = 0
    """
...
def genpca_parallel_test(arr, sigma=None, mask=None, patch_radius=2, pca_method='eig',
           tau_factor=None, return_sigma=False, out_dtype=None, processes=1, verbose=False):
    """General function to perform PCA-based denoising of diffusion datasets.
    ...
    for kx1 in range(0, arr_shape[2] - 2 * patch_radius):
        ...
        #                    patch_size ** 3, dim)
        resultslist=[]
        resultslist = pool.starmap_async(pca_patchloop_test,
...
def mppca(arr, mask=None, patch_radius=2, pca_method='eig', processes=1,
    ...
    if processes == 1:
        ...
        return genpca_parallel_test(arr, sigma=None, mask=mask, patch_radius=patch_radius,
                               pca_method=pca_method, tau_factor=None,
