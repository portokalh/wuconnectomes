...
if subject_processes>1:
    ...
    dwi_results = pool.starmap_async(dwi_preprocessing, [(dwipath, dwipath_preprocessed, subject, bvec_orient, denoise, savefa, function_processes,
                                     str_identifier, createmask, vol_b0, verbose) for subject in l]).get()
    tract_results = pool.starmap_async(create_tracts, [(dwipath_preprocessed, outtrkpath, subject, stepsize, function_processes,
    ...
    for subject in l:
       dwi_results.append(dwi_preprocessing(dwipath, dwipath_preprocessed, subject, bvec_orient, denoise, savefa,
                                         function_processes, str_identifier, createmask, vol_b0, verbose))
       tract_results.append(create_tracts(dwipath_preprocessed, outtrkpath, subject, stepsize, function_processes, str_identifier,
